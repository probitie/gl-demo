 ...
 */
void rd_check_errors(const std::string& context,const std::string & func_name, bool terminate_on_error);
...
{ \
    rd_check_errors("Before", #gl_api_call, rd_terminate_on_error); \
    gl_api_call; \
    rd_check_errors("After", #gl_api_call, rd_terminate_on_error); \
}
